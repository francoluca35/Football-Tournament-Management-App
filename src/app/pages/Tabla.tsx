...
export function Tabla() {
  ...
  const finalWinner = (() => {
    ...
    const finals = divisionMatches.filter(match => match.fixtureType === 'final');
    if (finals.length === 0) return null;
    const finalMatch = [...finals].sort((a, b) => {
  ...
  const downloadAsImage = async () => {
    ...
    try {
      const source = tableRef.current.cloneNode(true) as HTMLElement;
  ...
  const downloadBracketImage = async (elementId: string, fileSuffix: string) => {
    ...
    try {
      ...
      if (isFullBracket) {
        source.style.paddingTop = '56px';
        source.style.paddingBottom = '24px';
        source.querySelectorAll('h4').forEach(node => {
      ...
      const { scrollWidth, scrollHeight } = source;
      const finalWidth = Math.max(scrollWidth, 1200);
      source.style.width = `${finalWidth}px`;
      source.style.height = `${scrollHeight}px`;
      const canvas = await html2canvas(source, {
        backgroundColor: '#0f1115',
        scale: 2,
  ...
  const getPenaltyScoreLabel = (match: { homeScore?: number; awayScore?: number; penaltiesWinnerTeamId?: string; penaltiesHomeScore?: number; penaltiesAwayScore?: number; home: { teamId: string | null }; away: { teamId: string | null } }, side: 'home' | 'away') => {
    ...
    if (match.penaltiesWinnerTeamId && penScore !== undefined) {
      const winnerTag = teamId && match.penaltiesWinnerTeamId === teamId ? ' G' : '';
  ...
  const downloadMatchdayResults = async (matchday: number) => {
    ...
    try {
      const source = matchdayElement.cloneNode(true) as HTMLElement;
      source.style.backgroundImage =
        "linear-gradient(rgba(0,0,0,0.8), rgba(0,0,0,0.8)), url('/Assets/fondo-torneo.png')";
      source.style.backgroundSize = 'contain';
      source.style.backgroundPosition = 'center';
      ...
      source.style.padding = '24px';
      source.style.boxSizing = 'border-box';
      if (selectedCompetitionData?.tournamentType === 'copa') {
        source.style.minWidth = '1600px';
      } else {
        source.style.minWidth = '1200px';
      }
      document.body.appendChild(source);
      ...
      const { scrollWidth, scrollHeight } = source;
      const minWidth = selectedCompetitionData?.tournamentType === 'copa' ? 1600 : 1200;
      const finalWidth = Math.max(scrollWidth, minWidth);
      source.style.width = `${finalWidth}px`;
      source.style.height = `${scrollHeight}px`;
  ...
  const downloadMatchdayDateResults = async (matchday: number, date: string) => {
    const matchdayElement = document.getElementById(`matchday-${matchday}-${date}`);
    if (!matchdayElement) return;

    try {
      if (selectedCompetitionData?.tournamentType === 'copa') {
        const matchesForDate = divisionMatches.filter(match =>
          match.matchday === matchday
          && (match.date ?? 'Sin fecha') === date
        ...
        container.style.backgroundImage =
          "linear-gradient(rgba(0,0,0,0.78), rgba(0,0,0,0.78)), url('/Assets/fondo-torneo.png')";
        ...
        header.innerHTML = `
          <div style="font-size:22px;font-weight:700;">Fecha ${matchday}</div>
          <div style="font-size:14px;color:#b8b8b8;">${date} · ${divisions.find(d => d.id === selectedDivision)?.name ?? 'División'}</div>
        `;
        ...
        matchesForDate.forEach(match => {
          ...
          home.style.whiteSpace = 'normal';
          home.textContent = getTeamNameById(match.homeTeamId);

          const score = document.createElement('div');
          score.style.minWidth = '140px';
          ...
          away.style.fontWeight = '600';
          away.style.whiteSpace = 'normal';
          away.textContent = getTeamNameById(match.awayTeamId);
        ...
        const fileName = competitionName
          ? `${divisionName}-${competitionName}`.toLowerCase().replace(/\s+/g, '-')
          : divisionName.toLowerCase().replace(/\s+/g, '-');
        link.download = `fecha-${matchday}-${date}-${fileName}.jpg`;
        link.href = canvas.toDataURL('image/jpeg', 0.95);
        link.click();
        return;
      }
                                  ...
                                  <div className="flex items-center justify-between rounded-md border border-border/60 px-3 py-2 bg-card/60">
                                      ...
                                      {match.homeScore !== undefined && match.awayScore !== undefined && (
                                        <span className="text-sm font-semibold text-primary">
                                          {getPenaltyScoreLabel(match, 'home')}
                                        </span>
                                      )}
                                    </div>
                                    <div className="flex items-center justify-between rounded-md border border-border/60 px-3 py-2 bg-card/60">
                                      ...
                                      {match.homeScore !== undefined && match.awayScore !== undefined && (
                                        <span className="text-sm font-semibold text-primary">
                                          {getPenaltyScoreLabel(match, 'away')}
                                        </span>
                                      )}
                                    </div>
                                  </div>
          ...
          {matchdays.length > 0 && (
            <div className="space-y-4">
              ...
              {matchdays.map(matchday => {
                ...
                const matchesByDate = matchdayMatches.reduce<Record<string, typeof matchdayMatches>>((acc, match) => {
                  const key = match.date ?? 'Sin fecha';
                  if (!acc[key]) acc[key] = [];
                  acc[key].push(match);
                  return acc;
                }, {});
                const matchDates = Object.keys(matchesByDate).sort();
                ...
                return (
                  <div key={matchday} className="rounded-xl border border-border bg-card/80 overflow-hidden shadow-sm">
                    <div className="bg-accent/60 px-6 py-4 border-b border-border flex items-center justify-between">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        >
                          ...
                          Descargar fecha completa
                        </button>
                        {selectedCompetitionData?.tournamentType === 'copa' && matchDates.length > 1 && (
                          <>
                            {matchDates.map(date => (
                              <button
                                key={date}
                                onClick={() => downloadMatchdayDateResults(matchday, date)}
                                className="flex items-center gap-2 border border-border px-3 py-1.5 rounded-lg text-foreground hover:bg-accent transition-colors text-sm"
                              >
                                <Download className="w-4 h-4" />
                                {date}
                              </button>
                    ...
                    <div id={`matchday-${matchday}`} className="p-6 bg-card space-y-6">
                      <div className="flex items-center gap-3">
                        <Trophy className="w-6 h-6 text-primary" />
                      ...
                      {matchDates.map(date => (
                        <div key={date} id={`matchday-${matchday}-${date}`} className="space-y-3">
                          ...
                          {matchesByDate[date].map(match => {
                            ...
                            return (
                              <div
                                key={match.id}
                                className="flex flex-col gap-3 p-4 bg-accent/60 rounded-lg"
                              >
                                <div className="flex items-center justify-between gap-4">
                                ...
                                <div className="flex justify-center">
                                  <span className="text-xs text-muted-foreground bg-card border border-border px-2 py-1 rounded-full">
