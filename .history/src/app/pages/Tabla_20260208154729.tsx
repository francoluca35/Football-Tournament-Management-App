...
export function Tabla() {
  ...
  const buildBracketRounds = () => {
    ...
    type BracketMatch = { id: string; home: BracketTeam; away: BracketTeam };
    ...
    while (current.length > 0) {
      rounds.push({ title: roundTitleFor(current.length), matches: current });
            ...
            <div className="space-y-6">
              {groupEntries.length === 0 ? (
                ...
                <div className="rounded-xl border border-border bg-card/80 p-6 shadow-sm">
                  ...
                  <div className="overflow-x-auto">
                 <div className="grid grid-cols-7 gap-12 min-w-[1100px] relative">
                      {bracketRounds.map((round, roundIndex) => (
                        <div key={`${round.title}-${roundIndex}`} className="min-w-[220px]">
                          ...
                          <div className="space-y-6">
                            {round.matches.map((match) => {
                              const homeName = match.home.teamId
                                ? teams.find(t => t.id === match.home.teamId)?.name
                                : match.home.label;
                              const awayName = match.away.teamId
                                ? teams.find(t => t.id === match.away.teamId)?.name
                                : match.away.label;
                              ...
                              return (
                                <div key={match.id} className="space-y-2">
                                  <div className="flex items-center justify-between rounded-md border border-border/60 px-3 py-2 bg-card/60">
                                    ...
                                    <span className="text-[10px] text-muted-foreground">{match.home.label}</span>
                                  </div>
                                  <div className="flex items-center justify-between rounded-md border border-border/60 px-3 py-2 bg-card/60">
                                    <span className="text-sm font-medium text-foreground">{awayName}</span>
                                    <span className="text-[10px] text-muted-foreground">{match.away.label}</span>
                                  </div>
                                </div>
                              );
                            })}
